...
- [x] Screenshots. Both stores want real captures, not mockups. Shoot the
      ...
      5. Settings, then Anki, with the fields mapped. *(store)*
      6. Settings, then Words, with a real list and real counts. *(store)*
...
- [x] Privacy policy is actually live at the URL above (GitHub Pages must
      be switched on first, Settings, then Pages, then main /docs).
