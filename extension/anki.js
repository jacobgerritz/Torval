...
var TorvalAnki = (function () {
  ...
  /** This add-on's own address, which is what AnkiConnect is checking. */
  function here() {
    try {
      return (typeof location !== 'undefined' && location.origin) || 'this add-on';
    } catch (err) {
      return 'this add-on';
    }
  }
    ...
    try {
      ...
      if (res.status === 403) {
        throw new Error('AnkiConnect refused this because of where it came ' +
          ...
          'and add "' + here() + '" to webCorsOriginList.');
      }
